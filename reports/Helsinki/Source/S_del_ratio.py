...
sources = (5, 10, 20, 30, 50, 70, 100, 200, 300, 500)
...
for x in xrange(len(flood)):
	...
	for source in sources:
		del_ratio = []
		for j in xrange(1, apps+1):
			fname = '%s_%s/S-%d/Source_S-%d_%s_F-%s_MultiAppChecks-App%d.txt' % (scheme[x], relay[x], source, source, relay[x], flood[x], j)
		...
		print '%s %.2f %.2f App%f %f App%f' % (source, avg, maximum[0], maximum[1], minimum[0], minimum[1])
		file.write(str(source)+' '+str(avg)+' '+str(maximum[0])+' '+str(maximum[1])+' '+str(minimum[0])+' '+str(minimum[1])+'\n')
	file.close()
	...
	for stat in cdf:
		for source in sources:
			file = open('Cumulative_'+stat+'_Percentage_S-'+str(source)+'_'+scheme[x]+'_'+relay[x]+'.txt', 'w')
			file.write('time avg max app min app\n')
			for i in xrange(0, len(intervals)):
				del_ratio = []
				for j in xrange(1, apps+1):
					fname = '%s_%s/S-%d/Source_S-%d_%s_F-%s_CDF%sReport-App%d.txt' % (scheme[x], relay[x], source, source, relay[x], flood[x], stat, j)
