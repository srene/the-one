...
public class Zipf
{
   ...
   public Zipf(int range, long seed, boolean shuffle_numbers, double exp){
   	sa = shuffle_numbers;
   	exponent = exp;
   	r = new Random(seed);
	init(range);
   ...
   public Zipf(int range, long seed, double exp){
   	exponent = exp;
   	r = new Random(seed);
	init(range);
   }

   public Zipf(int range, boolean shuffle_numbers, double exp){
   	sa = shuffle_numbers;
   	exponent = exp;
   	r = new Random();
	init(range);
   ...
   public Zipf(int range, double exp){
   	exponent = exp;
   	r = new Random();
	init(range);
   }
	...
	for (k=1;k<=n;k++){
	    sum += (1.0/(double)Math.pow(k,exponent));
	}
	...
	return 1.0/(Math.pow(d,exponent)*h);
